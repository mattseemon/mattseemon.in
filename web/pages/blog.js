...
export default class Blog extends React.Component {
    ...
    static async getInitialProps({ query }) {
        ...
        let allPosts = (ctx => {
            ...
            let data = keys.map((key, index) => {
                let slug = 'posts/' + key.replace(/^.*[\\\/]/, '').split('.').slice(0, -1).join('.');
        ...
        let posts = allPosts.splice(startIndex, 5);
    ...
    renderPagination(totalPages, currentPage) {
        ...
        pageLinks.push(
            <li className={ this.getClassNames({ disabled:currentPage === 1 }) } key='first'>
                <Link href={{ pathname: `/blog/${ currentPage-1 }` }}>
    ...
    render() {
        ...
        return (
            <Page title="Blog" subTitle="Random musing of a confused soul!" >
                        ...
                        <div>
                            {posts.map((post, index) => (
                                <div key={index}>
                                    ...
                                    <p><Link href={{ pathname: `/blog/${post.slug}`}} ><a>read more...</a></Link></p>
                                </div>
                            ))}
                            { totalPages > 1 &&  
